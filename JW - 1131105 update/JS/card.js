...
let cardB = new Card ("img/author.jpg","前端React框架 | 待辦清單",false,"React框架","網頁框架實作。","練習程式手感作品。");
...
cards.forEach(item => {
    ...
    <p>${item.txt}</p>
    <p>${item.content}</p>
    <div class="tagArea"><span class="tag">${setNumber(item.Number)}</span><span class="tag">${item.tag}</span></div>`;
